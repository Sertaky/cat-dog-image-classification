"""DataLoader construction for ResNet18 transfer-learning experiments."""
...
def build_resnet18_transfer_dataloaders(
    batch_size: int = 32,
    num_workers: int = 4,
    pin_memory: bool = True,
) -> tuple[DataLoader[Any], DataLoader[Any], DataLoader[Any]]:
    """Build train, validation, and test loaders with ResNet18 preprocessing."""
    ...
    train_dataset = CatDogDataset(
        "splits/train.csv",
        transform=build_resnet18_train_transform(),
    )
    val_dataset = CatDogDataset(
        "splits/val.csv",
        transform=build_resnet18_eval_transform(),
    )
...
def main() -> None:
    ...
    train_loader, val_loader, test_loader = build_resnet18_transfer_dataloaders()
