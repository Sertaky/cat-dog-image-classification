"""Train the SimpleCNNGAPBatchNorm architecture experiment."""
...
from src.models import SimpleCNNGAPBatchNorm
from src.training import train_one_epoch, validate_one_epoch


PROJECT_ROOT = Path(__file__).resolve().parents[1]
CHECKPOINT_PATH = PROJECT_ROOT / "checkpoints" / "simple_cnn_gap_bn_best.pt"
...
def save_checkpoint(
    *,
    ...
    """Save the best BatchNorm GAP model and its complete training state."""
    ...
    torch.save(
        {
            ...
            "model_name": "SimpleCNNGAPBatchNorm",
...
def run_smoke_verification(
    *,
    ...
    model = SimpleCNNGAPBatchNorm().to(device)
...
def run_training(
    *,
    ...
    """Run the complete BatchNorm GAP training and validation experiment."""
    set_random_seed(random_seed)
    pin_memory = device.type == "cuda"
    train_loader, val_loader, _ = build_classification_dataloaders(
        batch_size=batch_size,
...
def main() -> None:
    """Configure and run smoke verification or the full BatchNorm experiment."""
    random_seed = 42
    batch_size = 32
    num_workers = 4
    learning_rate = 0.001
