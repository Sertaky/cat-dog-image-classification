"""Train a frozen-backbone ImageNet-pretrained ResNet18 classifier."""
...
CHECKPOINT_PATH = PROJECT_ROOT / "checkpoints" / "resnet18_frozen_best.pt"
EXPECTED_TOTAL_PARAMETERS = 11_177_538
EXPECTED_TRAINABLE_PARAMETERS = 1_026
...
def verify_frozen_configuration(model: ResNet18Transfer) -> tuple[int, int, int]:
    """Validate parameter counts and trainability for frozen-backbone training."""
    ...
    if model.resnet.conv1.weight.requires_grad:
        raise AssertionError("conv1.weight must be frozen")
...
def trainable_parameters(model: nn.Module) -> list[nn.Parameter]:
    """Return only parameters that the optimizer should update."""
    parameters = [
...
def save_checkpoint(
    *,
    ...
    """Save the best frozen-backbone model and training state."""
...
def run_smoke_verification(
    *,
    ...
    """Verify the frozen transfer-learning setup with one optimization batch."""
    ...
    model = ResNet18Transfer(freeze_backbone=True).to(device)
    total, trainable, frozen = verify_frozen_configuration(model)
    criterion = nn.CrossEntropyLoss()
    optimizer = Adam(trainable_parameters(model), lr=learning_rate)
    CHECKPOINT_PATH.parent.mkdir(parents=True, exist_ok=True)
    ...
    if not backbone_unchanged:
        raise AssertionError("Frozen conv1.weight changed during smoke verification")
    ...
    print(f"Frozen conv1.weight unchanged: {backbone_unchanged}")
...
def run_training(
    *,
    ...
    """Run the complete frozen-backbone training and validation experiment."""
    set_random_seed(random_seed)
    pin_memory = device.type == "cuda"
    train_loader, val_loader, _ = build_resnet18_transfer_dataloaders(
        batch_size=batch_size,
    ...
    if not backbone_unchanged:
        raise AssertionError("Frozen conv1.weight changed during training")
    if not classifier_was_updated:
        raise AssertionError("Classifier parameters did not change during training")
...
def main() -> None:
    """Configure and run smoke verification or frozen-backbone training."""
    random_seed = 42
    batch_size = 32
    num_workers = 4
    learning_rate = 0.001
