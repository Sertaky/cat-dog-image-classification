"""Train the SimpleCNNGAP architecture experiment."""
...
from src.models import SimpleCNNGAP
from src.training import train_one_epoch, validate_one_epoch


PROJECT_ROOT = Path(__file__).resolve().parents[1]
CHECKPOINT_PATH = PROJECT_ROOT / "checkpoints" / "simple_cnn_gap_best.pt"
...
def save_checkpoint(
    *,
    ...
    """Save the best GAP model and its complete training state."""
...
def run_training(
    *,
    ...
    """Run the complete GAP training and validation experiment."""
    set_random_seed(random_seed)
    pin_memory = device.type == "cuda"
    train_loader, val_loader, _ = build_classification_dataloaders(
        batch_size=batch_size,
...
def main() -> None:
    """Configure and run smoke verification or the full GAP experiment."""
    random_seed = 42
    batch_size = 32
    num_workers = 4
    learning_rate = 0.001
